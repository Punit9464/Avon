...
class stats extends AvonCommand{
    ...
    async run(client,message,args,prefix){
        ...
        let b1 = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel(`${Math.round(client.ws.ping)} ms`).setEmoji(`${client.emoji.ping}`).setDisabled(true).setCustomId(`lolok`);
        let b2 = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel(`${client.guilds.cache.reduce((a,b) => a + b.memberCount,0)} Users`).setEmoji(`${client.emoji.users}`).setDisabled(true).setCustomId(`lolbhai`);
        let b3 = new ButtonBuilder().setStyle(ButtonStyle.Danger).setLabel(`${client.guilds.cache.size} Servers`).setEmoji(`${client.emoji.ping}`).setDisabled(true).setCustomId(`bc`);
